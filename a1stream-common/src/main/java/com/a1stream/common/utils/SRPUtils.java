...
public class SRPUtils {
    ...
    public static byte[] calculatePasswordClaimSignature(String name, String password, String timestamp, BigInteger a, BigInteger A, BigInteger B, BigInteger salt, byte[] secretBlock) {
        BigInteger x = calculateX(name, password, salt);
        // u = H(A, B)
        ...
        byte[] passwordClaimSignature = HashUtils.hmac(key, name.getBytes(StandardCharsets.UTF_8), secretBlock, timestamp.getBytes(StandardCharsets.UTF_8));
