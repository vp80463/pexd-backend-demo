...
import com.a1stream.common.ifs.messageing.listener.SimpleMessageListenerContainer;
...
public class AmazonSQSListenerAnnotationBeanPostProcessor implements BeanPostProcessor, BeanFactoryAware {
    ...
    private void processAmazonSQSListener(AmazonSQSListener annotation, Object bean, Method method) {
        ...
        if(!StringUtils.isEmpty(queueName)){
            queueUrl = sqsMessageUtil.createSqsQueue(queueName);
        }

        int waitTimeSeconds = annotation.waitTimeSeconds();
        ...
        Assert.hasText(queueUrl, "amazon simple queue service name can not be null");
        SimpleMessageListenerContainer listenerContainer = containerFactory.createListenerContainer(queueUrl,waitTimeSeconds,maxNumberOfMessages, message -> useMethod.invoke(bean, message));
